...
export class ColisForm {
    constructor() {
        ...
        this.form = document.querySelector("form[data-colis-form]");
        this.init();
    }
    init() {
        if (!this.form)
            return;
        const poidsInput = this.getElement("#poids-colis");
        const typeTransportSelect = this.getElement("#type-cargaison");
        ...
        const btnCalculer = document.querySelector("[data-btn-calculer]");
    ...
    calculerPrix() {
        const poidsInput = this.getElement("#poids-colis");
        const typeTransportSelect = this.getElement("#type-cargaison");
        const prixInput = this.getElement("#prix-calcule");
        if (!poidsInput || !typeTransportSelect || !prixInput)
            return;
    ...
    async collectFormData() {
        ...
        const valeurDeclaree = valeurDeclareeStr !== "" ? parseFloat(valeurDeclareeStr) : undefined;
        return {
            client: {
    ...
    async checkCargaisonFermee(typeCargaison) {
        try {
            const response = await fetch('/api/cargaisons');
            if (!response.ok)
                return false;
            const cargaisons = await response.json();
            const cargaison = cargaisons.find((c) => c.id === typeCargaison || c.type === typeCargaison);
    ...
    async handleSubmit(event) {
        ...
        try {
            ...
            const colis = {
                ...
                typeCargaison: formData.typeCargaison,
    ...
    showMessage(message, type) {
        ...
        notification.className = `message-notification fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg max-w-sm ${type === "success" ? "bg-green-500 text-white" : "bg-red-500 text-white"}`;
...
//# sourceMappingURL=ColisForm.js.map