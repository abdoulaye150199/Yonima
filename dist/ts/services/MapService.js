...
export class MapService {
    ...
    addShipmentMarkers(containerId, cargaisons) {
        const map = this.maps.get(containerId);
        if (!map || !this.isLeafletAvailable())
            return;
    ...
    clearShipmentMarkers(containerId) {
    ...
    focusOnShipment(containerId, cargaison) {
        const map = this.maps.get(containerId);
        if (!map)
            return;
        const markers = this.shipmentMarkers.get(containerId) || [];
    ...
    showInfoPanel(cargaison, position) {
        ...
        if (elements.title)
            elements.title.textContent = cargaison.id;
        if (elements.type)
        ...
        if (elements.progress)
            elements.progress.textContent = `${this.getProgressionByStatus(cargaison.statut)}%`;
        if (elements.distance)
            elements.distance.textContent = this.calculateDistance(cargaison.origine, cargaison.destination);
        if (elements.coordinates)
            elements.coordinates.textContent = `${position.lat.toFixed(1)}°N, ${Math.abs(position.lng).toFixed(1)}°W`;
        if (elements.speed)
            elements.speed.textContent = this.getVitesseByType(cargaison.type);
    ...
    calculateDistance(origine, destination) {
        const distances = {
    ...
    searchShipmentOnMap(containerId, searchTerm) {
        const markers = this.shipmentMarkers.get(containerId) || [];
        const foundMarker = markers.find(marker => marker.shipmentData.id.toLowerCase().includes(searchTerm.toLowerCase()));
        if (foundMarker) {
            this.focusOnShipment(containerId, foundMarker.shipmentData);
        }
    }
    // Export des positions
    exportPositions(containerId) {
        const markers = this.shipmentMarkers.get(containerId) || [];
