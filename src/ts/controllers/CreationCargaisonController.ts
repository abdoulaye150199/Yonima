...
export class CreationCargaisonController {
    private static instance: CreationCargaisonController;
    private produits: Produit[] = [];
    private produitCounter = 0;
    ...
    public static getInstance(): CreationCargaisonController {
    ...
    private setupPriceCalculation(): void {
        ...
        const typeSelect = document.getElementById('type-cargaison') as HTMLSelectElement;
        if (typeSelect) {
    ...
    private handleTypeChange(): void {
        ...
        if (type === 'Aérien') document.getElementById('numero-vol-div')?.classList.remove('hidden');
        if (type === 'Maritime') document.getElementById('numero-navire-div')?.classList.remove('hidden');
        if (type === 'Routier') document.getElementById('numero-camion-div')?.classList.remove('hidden');
    }

    // Mettre à jour l'indicateur de capacité
    private updateCapacityIndicator(): void {
    ...
    private exposeGlobalFunctions(): void {
        (window as any).ajouterProduit = () => this.ajouterProduit();
        (window as any).supprimerProduit = (id: number) => this.supprimerProduit(id);
    }

    // Ajouter un produit
    public ajouterProduit(): void {
        ...
        setTimeout(() => {
            const firstInput = modal.querySelector('input[name="nom-produit"]') as HTMLInputElement;
            if (firstInput) firstInput.focus();
        }, 100);
    }

    // Créer le modal d'ajout de produit
    private creerModalProduit(): HTMLElement {
    ...
    private setupModalEvents(modal: HTMLElement): void {
        ...
        const productForm = modal.querySelector('#productForm') as HTMLFormElement;
        if (productForm) {
    ...
    private confirmerAjoutProduit(modal: HTMLElement, form: HTMLFormElement): void {
        const formData = new FormData(form);
        
        const produit: Produit = {
            id: ++this.produitCounter,
            nom: formData.get('nom-produit') as string,
            type: formData.get('type-produit') as string,
            poids: parseFloat(formData.get('poids-produit') as string),
            valeur: parseFloat(formData.get('valeur-produit') as string) || 0,
            description: formData.get('description-produit') as string || ''
        };
    ...
    // Fermer le modal
    private fermerModal(modal: HTMLElement): void {
    ...
    public supprimerProduit(id: number): void {
    ...
    private updateProduitsDisplay(): void {
        ...
        if (this.produits.length === 0) {
            ...
            if (resumeDiv) resumeDiv.classList.add('hidden');
        } else {
    ...
    private updateResumeDisplay(): void {
        const resumeDiv = document.getElementById('resume-produits');
        if (!resumeDiv) return;
    ...
    private async handleFormSubmit(e: Event): Promise<void> {
        ...
        try {
            ...
            const cargaisonData = {
                ...
                numeroVol: formData.get('numeroVol') as string || '',
                numeroNavire: formData.get('numeroNavire') as string || '',
                numeroCamion: formData.get('numeroCamion') as string || '',
                ...
                dureeEstimee: formData.get('dureeEstimee') as string || '',
    ...
    private validateForm(formData: FormData): { isValid: boolean; errors: string[] } {
        const errors: string[] = [];
        ...
        for (const field of requiredFields) {
            ...
            if (!value || value.trim() === '') {
                ...
                const fieldElement = document.querySelector(`[name="${field.name}"]`) as HTMLElement;
        ...
        const poidsMax = parseFloat(formData.get('poidsMax') as string);
        const prixParKg = parseFloat(formData.get('prixParKg') as string);
