...
export class PaginationUtil {
  ...
  private onPageChange: (page: number, startIndex: number, endIndex: number) => void;
  private cssClasses: Required<NonNullable<PaginationConfig['cssClasses']>>;
  ...
  public setup(totalItems: number, targetElement: HTMLElement): void {
  ...
  private show(targetElement: HTMLElement): void {
  ...
  private createPaginationContainer(): HTMLElement {
    ...
    const prevBtn = container.querySelector(`#${this.containerId}-prev`) as HTMLButtonElement;
    const nextBtn = container.querySelector(`#${this.containerId}-next`) as HTMLButtonElement;
  ...
  private updateControls(): void {
    ...
    if (startEl) startEl.textContent = startItem.toString();
    if (endEl) endEl.textContent = endItem.toString();
    if (totalEl) totalEl.textContent = this.totalItems.toString();
    
    // Mettre à jour les boutons
    const prevBtn = document.getElementById(`${this.containerId}-prev`) as HTMLButtonElement;
    const nextBtn = document.getElementById(`${this.containerId}-next`) as HTMLButtonElement;
  ...
  private updatePageNumbers(totalPages: number): void {
  ...
  public refresh(totalItems: number): void {
    this.totalItems = totalItems;
    if (totalItems <= this.itemsPerPage) {
      this.hide();
      this.onPageChange(1, 0, totalItems);
